...
TRACE is strict drift.trace/v1 JSONL. Use '-' to read it from stdin.
";
...
fn render_command(args: Vec<OsString>) -> Result<()> {
    ...
    let bytes = read_bounded_file(Path::new(&args[0]), MAX_FILE_BYTES * 2)?;
...
fn read_bounded_file(path: &Path, limit: usize) -> Result<Vec<u8>> {
    let mut file = File::open(path).map_err(|error| DriftError::io("open file", error))?;
...
fn write_atomic(path: &Path, bytes: &[u8]) -> Result<()> {
    ...
    let result = (|| {
        ...
        let mut file = options
            .open(&temporary)
            .map_err(|error| DriftError::io("create temporary report", error))?;
        file.write_all(bytes)
            .and_then(|_| file.write_all(b"\n"))
            .and_then(|_| file.sync_all())
            .map_err(|error| DriftError::io("write temporary report", error))?;
        fs::rename(&temporary, path)
            .map_err(|error| DriftError::io("replace report atomically", error))
    })();
    if result.is_err() {
...
#[cfg(test)]
mod tests {
    use super::{JUDGMENT_SCHEMA, REPORT_SCHEMA, TRACE_SCHEMA, parse_analyze_args};
